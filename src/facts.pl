...
required_fragment(segel_lurien).
required_fragment(segel_herrah).
required_fragment(segel_monomon).

initial_enemy_in_area(greenpath, husk).
initial_enemy_in_area(black_egg_temple, the_hollow_knight).

% enemy_template(Nama, MaxHP, Damage)
